...
DEFAULT_WEIGHTS = 'output/MultiResGRUNet/default_model/weights.20000.npy'
...
def load_demo_images():
    ims = []
    for i in range(3):
        crop_img = cv2.imread('imgs/%d.png' % i)
        # crop_img = image[5:132, 5:132, :]
        # im = Image.open('imgs/%d.png' % i)
        ims.append([np.array(crop_img).transpose(
...
def main():
    ...
    NetClass = load_model('MultiResGRUNet')
