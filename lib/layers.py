...
class Unpool3DLayer(Layer):
    ...
    def set_output(self):
        ...
        unpooled_output = tensor.set_subtensor(
            unpooled_output[:, padding[0]:output_shape[1] + padding[0]:unpool_size[0], :,
            padding[1]:output_shape[3] + padding[1]:unpool_size[
...
class FCConv3DLayer(Layer):
    ...
    def __init__(self, prev_layer, fc_layer, filter_shape, padding=None, params=None):
        ...
        self._filter_shape = [filter_shape[0],  # out channel 128
                              filter_shape[2],  # time 128
                              filter_shape[1],  # in channel 3
                              filter_shape[3],  # height 3
                              filter_shape[4]]  # width 3
        self._padding = padding
